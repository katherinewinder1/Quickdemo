    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if isinstance(place.ant, ContainerAnt) and place.ant.can_contain(self):
                ...
                place.ant = self
                # places.self should be self (the bodyguard)
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        """
        # thrower ants should throw at the closest bee in front of it within range
        # BEGIN Problem 3 and 4
        ant_loc = self.place
        counter = 0
        while ant_loc is not beehive and ant_loc:
            if len(ant_loc.bees) != 0:
                if counter <= self.max_range and counter >= self.min_range:
                    return random_or_none(ant_loc.bees)
                elif counter > self.max_range:
                    return None
            ant_loc = ant_loc.entrance
            counter += 1
        # END Problem 3 and 4
...
class ShortThrower(ThrowerAnt):
    ...
    # BEGIN Problem 4
    implemented = True
    short_thrower = ThrowerAnt('ShortThrower')   # Change to True to view in the GUI
    # END Problem 4
...
class LongThrower(ThrowerAnt):
    ...
    # BEGIN Problem 4
    implemented = True
    long_thrower = ThrowerAnt('LongThrower')   # Change to True to view in the GUI
    # END Problem 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        if self.armor <= 0:
            bee_damage += self.damage
        for bee in bees_copy:
            bee.reduce_armor(bee_damage)
        if self.armor <= 0:
            self.place.remove_insect(self)
            self.death_callback()
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting > 0:
            ...
            if self.place.bees is not None and self.place.bees:
...
class BodyguardAnt(ContainerAnt):
    ...
    implemented = True
    def __init__(self):
        ContainerAnt.__init__(self)
        self.armor = 2  # Change to True to view in the GUI
    # END Problem 9
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.imposter:
            ...
            current_loc = self.place
            ThrowerAnt.action(self, gamestate)
            while current_loc.exit:
                behind_ants = current_loc.exit.ant
                if behind_ants and behind_ants not in self.doubled_ant:
                    behind_ants.damage *= 2
                    self.doubled_ant.append(behind_ants)
                if isinstance(behind_ants, ContainerAnt) and behind_ants and behind_ants.contained_ant and behind_ants.contained_ant not in self.doubled_ant:
                    behind_ants.contained_ant.damage *= 2
                    self.doubled_ant.append(behind_ants.contained_ant)
                current_loc = current_loc.exit
        # END Problem 13

    def remove_from(self, place):
        if isinstance(self, QueenAnt) and not self.imposter:
            return
        else:
            Ant.remove_from(self, place)
    def reduce_armor(self, amount):
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        if target:
            apply_effect(make_slow, target, 1)
        # END Problem EC
